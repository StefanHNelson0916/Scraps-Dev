...
def results(search_type, search_input, dish_type, order_by):
    ...
    if len(recipes) != 0:
        ...
        if user_id is not None:
            ...
            in_history = check_in_history(item_values)
            if in_history:
                db.add_user_history_item(item_values)
...
def check_in_history(values : dict):
    occurred_at = str(datetime.now().date())
    values['occurred_at'] = occurred_at
    values['item_type'] = 'Searched:Ingredients'
    values['item_content'] = values['search_input']
    in_history = db.check_already_in_history(values)
    if not in_history:
        values['occurred_at'] = datetime.now()
        db.add_user_history_item(values)
